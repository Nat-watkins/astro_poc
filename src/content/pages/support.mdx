---
title: "Support"
description: ""
---

## Join the Community

Get the latest updates from the Blue Button 2.0 API team and talk with fellow developers in the [Google Developer Group for CMS Blue Button 2.0 API](https://groups.google.com/g/developer-group-for-cms-blue-button-api).

## Contact Us

To get help with questions specific to your integration or ask questions about the production API approval process, email [BlueButtonAPI@cms.hhs.gov](mailto:BlueButtonAPI@cms.hhs.gov). We respond to emails within 3 business days.


